...
def _post(self, soft=True):
    ...
    for invoice in self.filtered(lambda move: move.is_invoice(include_receipts=True)):
        ...
        if not invoice.invoice_date:
            if invoice.is_sale_document(include_receipts=True):
                ...
                with self.env.protecting([self._fields['invoice_currency_rate']],
                                         invoice) if is_manual_rate else nullcontext():
    ...
    for move in self:
        ...
        if move.line_ids.account_id.filtered(lambda account: not account.active) and not self.env.context.get(
    ...
    for move in to_post:
        ...
        if lock_dates:
            move.date = move._get_accounting_date(move._get_accounting_date_source(), affects_tax_report,
    ...
    for invoice in to_post:
        ...
        wrong_lines = invoice.is_invoice() and invoice.line_ids.filtered(lambda aml:
                                                                         ...
                                                                         and aml.display_type not in ('line_section',
                                                                                                      'line_subsection',
                                                                                                      'line_note')
                                                                         )
        if wrong_lines:
    ...
    for aml in self.line_ids:
        for partials, counterpart_field in [(aml.matched_debit_ids, 'debit_move_id'),
    ...
    if not self.env.user.has_group('account.group_partial_purchase_deductibility') and \
            self.filtered(lambda move: move.move_type == 'in_invoice' and move.invoice_line_ids.filtered(
                lambda l: l.deductible_amount != 100)):
    ...
    if non_deductible_lines := self.line_ids.filtered(
            lambda line: (line.display_type in ('non_deductible_product_total', 'non_deductible_tax'))):
    ...
    draft_reverse_moves.reversed_entry_id._reconcile_reversed_moves(draft_reverse_moves,
                                                                    self.env.context.get('move_reverse_cancel', False))
