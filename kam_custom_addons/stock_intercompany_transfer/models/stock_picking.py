...
class StockPickingInherit(models.Model):
    ...
    is_backorder_button_clicked = fields.Boolean(default=False,
                                                 string='Backorder'
                                                        ' Button Clicked')
    sub_vendor_id = fields.Many2one(comodel_name="res.partner", string="Order For", store=True,redonly=False, required=False,compute="compute_sub_vendor_id" )
    order_tag_ids = fields.Many2many(comodel_name="custom.order.tag", string="Order Tag", compute="compute_sub_vendor_id")
    ref_po_template_id = fields.Many2one('purchase.order.template',string='Ref Purchase template',compute="compute_sub_vendor_id")


    @api.depends('sale_id.sub_vendor_id', 'sale_id.order_tag_ids', 'purchase_id.sub_vendor_id', 'purchase_id.order_tag_ids',)
